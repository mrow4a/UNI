...
def detected_tags_to_cars(database):
    ...
    database.commit()
    cars_list.append(dict(zip(columns, ("plate_number",float(132)) )))
    return cars_list
    ...
    try:
        ...
        if (response and response['HTTPRESPONSE']=="1" and response['Eth0']):
            ...
            if (response2):
                ...
                #if you keep this filename path, please do not change crontab file in raspberry
        ...
        if (block==True):
            if (response['BLOCK_THREADS']=="Release"):
                if sock:
                    sock.close()
                block = "Release"
        elif (response['BLOCK_THREADS']=="Release"):
            if sock:
                sock.close()
...
def pre_init(args):
    ##MAIN## PLEASE DO NOT CHANGE THE NAME SPECIFIED IN LAMP_CONTROL
...
def main(args, session):
    ...
    while (True):
        ERROR_REPORT, BLOCK_THREADS = check_error_status()
        BLOCK_THREADS = error_to_server(session,BLOCK_THREADS,ERROR_REPORT) 
        if(BLOCK_THREADS == "Release"):
            reload(sys.modules['Car_discovery'])
            print "RELOADED CAR DETECTION MODULE"
            session = pre_init(args)
